...
function initApp() {
  ...
  // Initialize Global WhatsApp Floating Concierge Widget
  try {
    initGlobalWhatsAppWidget();
  } catch (e) {}
}

function initGlobalWhatsAppWidget() {
  if (document.getElementById('global-whatsapp-btn')) return;
  const legacy = document.querySelector('.whatsapp-floating');
  if (legacy) {
    legacy.remove();
  }
  const a = document.createElement('a');
  a.href = 'https://wa.me/971527582293?text=Hello%20Star%20Plus%20Travels,%20I%20would%20like%20to%20inquire%20about%20a%20booking.';
  a.target = '_blank';
  a.rel = 'noopener noreferrer';
  a.id = 'global-whatsapp-btn';
  a.className = 'fixed bottom-6 right-6 z-50 flex items-center justify-center w-14 h-14 bg-[#25D366] hover:bg-[#20ba59] text-white rounded-full shadow-2xl hover:scale-110 active:scale-95 transition-all duration-300 group cursor-pointer';
  a.setAttribute('aria-label', 'Chat on WhatsApp with Star Plus Travels');
  a.innerHTML = `
    <!-- Micro Pulse Ring -->
...
window.initGlobalWhatsAppWidget = initGlobalWhatsAppWidget;
