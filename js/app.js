  ...
  container.innerHTML = `
    <div class="testimonial-slide flex flex-col md:flex-row items-center gap-6 md:gap-8 text-left">
      ...
      <div class="relative shrink-0">
        <div class="w-20 h-20 md:w-24 md:h-24 rounded-full bg-slate-800 border-2 border-amber-500/30 flex items-center justify-center shadow-xl shadow-amber-500/10">
          <span class="text-2xl md:text-3xl font-bold text-amber-400 tracking-wider">${t.initials}</span>
        </div>
        <span class="absolute -bottom-1 -right-1 w-7 h-7 rounded-full bg-[#070B14] border border-slate-700 flex items-center justify-center text-xs text-white font-semibold shadow-md" title="${googleVerifiedText}">
          G
        </span>
      </div>
      ...
      <div class="flex-1">
        <div class="flex flex-wrap items-center gap-2 mb-3">
          <div class="flex items-center space-x-1 text-[#F59E0B] text-sm">
            ${Array(t.stars).fill('<i class="fa-solid fa-star"></i>').join('')}
          </div>
          <span class="inline-flex items-center gap-1.5 text-xs text-slate-300 font-semibold bg-slate-800/80 px-2.5 py-0.5 rounded-full border border-slate-700">
